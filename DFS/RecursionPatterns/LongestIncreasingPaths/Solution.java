class Solution {
//Runtime: 7 ms, faster than 93.71% of Java online submissions for Longest Increasing Path in a Matrix.
//Memory Usage: 40.5 MB, less than 87.76% of Java online submissions for Longest Increasing Path in a Matrix.
    ...
    private int dfs(int  matrix[][], int i,int j , int max,int prev, boolean visited[][],int dp[][]){
       ...
       //check curval less than prev then return
        ...
        max1 =1+dfs(matrix,i+1,j,max,prev,visited,dp);
        max2= 1+dfs(matrix,i-1,j,max,prev,visited,dp);
        max3= 1+dfs(matrix,i,j-1,max,prev,visited,dp);
        max4 =1+dfs(matrix,i,j+1,max,prev,visited,dp);
